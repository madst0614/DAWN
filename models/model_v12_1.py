"""
DAWN v12.1: SSM-guided Shared Neuron Architecture (v10 based)

Key features:
- SSM으로 토큰 중요도 계산
- 중요도 기반 뉴런 가중치 → 공유 compress/expand
- Attention in rank space (v10 style)
- compress/expand 둘 다 뉴런 라우팅

vs v12.0:
- v12.0: d_model Attention + Linear expand_Q/K/V
- v12.1: rank Attention + Neuron-based compress/expand
"""
...
class NeuronCircuit(nn.Module):
    """
    v12.1 Attention Layer (v10 based)

    SSM → 중요도 → 뉴런 가중치 → 공유 compress/expand
    Attention in rank space
    """
    def __init__(
        self,
        ...
        self.d_head = rank // n_heads  # v10 style: rank 기반
        self.rank = rank
        self.n_compress = n_compress
        self.n_expand = n_expand
    ...
    def forward(self, x, mask=None):
        ...
        # 7. Expand: 토큰별 expand 뉴런 선호도
        token_expand_pref = F.softmax(self.expand_router(attn_out), dim=-1)  # [B, S, n_expand]

        # 8. 중요도 × expand 뉴런선호 → expand 뉴런 가중치
        expand_weights = torch.einsum('bs,bsn->bn', importance, token_expand_pref)
        expand_weights = expand_weights / (expand_weights.sum(dim=-1, keepdim=True) + 1e-8)

        # 9. 공유 expand 행렬 생성
        # [B, n_expand] @ [n_expand, rank, d_model] → [B, rank, d_model]
        shared_expand = torch.einsum('bn,nrd->brd', expand_weights,
                                      self.shared_neurons.expand_neurons)

        # 10. 모든 토큰에 공유 expand 적용
        # [B, S, rank] @ [B, rank, d_model] → [B, S, d_model]
        output = torch.einsum('bsr,brd->bsd', attn_out, shared_expand)
...
class NeuronMemory(nn.Module):
    """v12.1: SSM-guided Knowledge Retrieval (v10 based)"""
    def __init__(
        self,
        shared_neurons: SharedNeurons,
        d_model: int,
    ...
    def forward(self, x):
        ...
        # 2. 토큰별 뉴런 선호
        token_compress_pref = F.softmax(self.compress_router(x), dim=-1)
...
class DAWN(nn.Module):
    ...
    vs v10:
    - 토큰별 라우팅 → SSM 기반 레이어 단위 라우팅
    - 개별 QKV → 공유 QKV

    vs v12.0:
    - d_model Attention → rank Attention
    - Linear expand → Neuron expand
    """
    __version__ = "12.1"
    ...
    def load_balance_loss(self, routing_infos):
        ...
        for layer_info in routing_infos:
            ...
            target_e = 1.0 / self.n_expand
            loss += ((expand_w.mean(dim=0) - target_e) ** 2).sum() * self.n_expand
            count += 1

            # Memory compress 뉴런 가중치
            mem_compress_w = layer_info['memory']['compress_weights']
            loss += ((mem_compress_w.mean(dim=0) - target_c) ** 2).sum() * self.n_compress
            count += 1
    ...
    def count_by_component(self):
        ...
        print(f"=== DAWN v12.1 Parameter Breakdown ===")
        ...
        print(f"---")
        print(f"Architecture: SSM → importance → shared compress/expand")
        print(f"Attention in rank space (d_head={self.rank // self.n_heads})")
        print(f"---")
