"""
DAWN v12.6: No SSM Ablation (Global Router only)

Ablation study: SSM 제거, 단순 projection으로 importance 계산

vs v12.5:
- SSM 제거
- context 강화 제거
- importance = softmax(importance_proj(x).squeeze(-1))

나머지는 v12.5와 동일:
- Global Router (5개)
- SharedNeurons
- NeuronCircuit, NeuronMemory
"""
...
class SharedNeurons(nn.Module):
    """v12.6: Shared neurons (same as v12.5)"""
    def __init__(
        self,
        d_model: int,
        rank: int,
...
class GlobalImportance(nn.Module):
    """
    v12.6: Simple projection for importance (no SSM)

    Replaces GlobalSSM with simple linear projection.
    ...
    def __init__(self, d_model: int):
        ...
        # Simple projection: x -> importance score
        self.importance_proj = nn.Linear(d_model, 1, bias=False)
    ...
    def forward(self, x):
        ...
        Args:
            x: [B, S, d_model]
        Returns:
            importance: [B, S] token importance (softmax normalized)
        """
        # Simple projection to scalar, then softmax
        scores = self.importance_proj(x).squeeze(-1)  # [B, S]
...
class GlobalRouters(nn.Module):
    """
    v12.6: Global routers (same as v12.5)
...
class NeuronCircuit(nn.Module):
    """v12.6: Attention circuit (same as v12.5)"""
    def __init__(
        self,
...
class DAWNBlock(nn.Module):
    """DAWN v12.6 block - uses global routers (no SSM)"""
    def __init__(
        self,
        shared_neurons: SharedNeurons,
        d_model: int,
        ...
        routing_info = {
            'attention': {
                **attn_routing,
                ...
                'neuron_weights': compress_w.detach(),
            },
            'memory': {
                **mem_routing,
                **knowledge_info,
                'neuron_weights': memory_w.detach(),
            },
        }
...
class DAWN(nn.Module):
    """
    DAWN v12.6: No SSM Ablation

    Ablation study to test SSM contribution:
    - No SSM (replaced with simple projection)
    - No context enhancement
    - Global Router preserved (same as v12.5)

    Expected comparison:
    - v12.5 > v12.6: SSM is helpful
    - v12.5 ≈ v12.6: SSM doesn't contribute much
    - v12.5 < v12.6: SSM is hurting performance
    """
    __version__ = "12.6"
    ...
    def __init__(
        ...
        knowledge_k: int = 10,
        knowledge_rank: int = None,
        state_dim: int = 64,  # kept for config compatibility, but not used
        dropout: float = 0.1,
        **kwargs
    ):
        super().__init__()
        ...
        self.state_dim = state_dim  # kept for compatibility
        ...
        # Global Importance (v12.6: simple projection, no SSM)
        self.global_importance = GlobalImportance(d_model)

        # Global Routers (same as v12.5)
    ...
    def forward(self, input_ids, labels=None, return_routing_info=False):
        ...
        # v12.6: Simple importance (no SSM, no context)
        importance = self.global_importance(x)  # [B, S]

        # No context enhancement (removed in v12.6)
    ...
    def count_by_component(self):
        ...
        # Global Importance (v12.6: simple projection)
        importance_params = self.global_importance.importance_proj.weight.numel()
        ...
        print(f"=== DAWN v12.6 Parameter Breakdown (No SSM) ===")
        ...
        print(f"Global Importance: {importance_params:,} ({importance_params/1e3:.1f}K) [simple projection]")
        print(f"Global Routers:    {routers:,} ({routers/1e3:.1f}K)")
        print(f"LayerNorms:        {norms:,} ({norms/1e3:.1f}K)")
        print(f"---")
        print(f"Architecture: Simple Importance (no SSM) → Global Routers")
        print(f"Attention in d_model space (d_head={self.d_model // self.n_heads})")
        print(f"---")
        print(f"vs v12.5: No SSM, no context enhancement")
        print(f"---")
    ...
    def get_config(self):
        return {
            ...
            'state_dim': self.state_dim,  # kept for compatibility
        }
