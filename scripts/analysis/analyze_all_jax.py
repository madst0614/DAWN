...
from scripts.analysis.performance_jax import (
    TextGeneratorJAX,
    SpeedBenchmarkJAX,
    ModelComparatorJAX,
...
class ModelAnalyzer:
    ...
    def _get_dataloader(self):
        """Get or create dataloader (returns validation tokens for JAX)."""
        if self._dataloader is None:
            from scripts.analysis.utils_jax import load_val_data_jax
            max_tokens = self.max_samples * 512  # Approximate
            self._dataloader = load_val_data_jax(self.val_data_path, max_tokens)
        return self._dataloader

    def _load_comparison_model(self):
        """Load comparison model (cached). Returns (model, params, name, config) or (None, None, None, None)."""
        if not self.compare_checkpoint:
            return None, None, None, None

        # Use cached model if available
        if hasattr(self, '_comparison_model') and self._comparison_model is not None:
            return self._comparison_model, getattr(self, '_comparison_params', None), self._comparison_model_name, getattr(self, '_comparison_config', {})

        try:
            from scripts.analysis.utils_jax import load_model_jax
            baseline_model, baseline_params, _, config = load_model_jax(self.compare_checkpoint)
            ...
            self._comparison_params = baseline_params
            self._comparison_model_name = baseline_name
            self._comparison_config = config

            return baseline_model, baseline_params, baseline_name, config
    ...
    def _cleanup_comparison_model(self):
        ...
        if hasattr(self, '_comparison_model') and self._comparison_model is not None:
            ...
            if hasattr(self, '_comparison_params'):
                del self._comparison_params
    ...
    def _analyze_multi_seed_checkpoints(self) -> Dict:
        ...
        from scripts.analysis.utils_jax import load_model_jax, evaluate_jax, load_val_data_jax, estimate_flops_jax, count_params_jax
        ...
        for source, checkpoint_path in all_checkpoints:
            ...
            try:
                ...
                if source == 'main' and cached_dawn_val.get('perplexity'):
                    ...
                    total_params = count_params_jax(self.params)
                    flops = estimate_flops_jax(self.config, seq_len=512)
                    result = {
                        'name': cp_name,
                ...
                # Load model
                model, params, tokenizer, config = load_model_jax(checkpoint_path)

                # Detect model type (check config since JAX models don't have attrs)
                is_dawn = config.get('n_feature_know', 0) > 0 or config.get('shared_neurons', 0) > 0
                ...
                # Estimate FLOPs
                flops = estimate_flops_jax(config)
                flops_G = flops / 1e9
    ...
    def _get_text_generator(self) -> TextGeneratorJAX:
        """Get or create TextGeneratorJAX instance."""
        if not hasattr(self, '_text_generator') or self._text_generator is None:
            self._text_generator = TextGeneratorJAX(self.tokenizer)
        return self._text_generator

    def _get_speed_benchmark(self) -> SpeedBenchmarkJAX:
        """Get or create SpeedBenchmarkJAX instance."""
        if not hasattr(self, '_speed_benchmark') or self._speed_benchmark is None:
            self._speed_benchmark = SpeedBenchmarkJAX()
        return self._speed_benchmark

    def _generate_text_simple(self, prompt: str, max_new_tokens: int = 50,
        ...
        generated_text, _ = generator.generate_from_text(
    ...
    def analyze_model_info(self) -> Dict:
        ...
        # Count parameters
        from scripts.analysis.utils_jax import count_params_jax, estimate_flops_jax
        total_params = count_params_jax(self.params)
        trainable_params = total_params  # All params are trainable in JAX by default
    ...
    def analyze_performance(self, n_batches: int = 200) -> Dict:
        ...
        """
        from scripts.analysis.utils_jax import evaluate_jax, load_val_data_jax, create_model_from_config
        ...
        val_results = evaluate_jax(
            model_instance, self.params, self.config, val_tokens,
            batch_size=32, seq_len=512
        )
        ...
        dawn_samples = self._generate_samples(max_new_tokens=self.gen_tokens, model=self.model, params=self.params, config=self.config, model_name="DAWN")
        ...
        if self.compare_checkpoint:
            ...
            try:
                vanilla_model, vanilla_params, vanilla_name, vanilla_config = self._load_comparison_model()
    ...
    def _benchmark_speed(self, warmup: int = 10, iterations: int = 50) -> Dict:
        ...
        return self._get_speed_benchmark().benchmark_forward(
            self.model, self.params, self.config,
            n_warmup=warmup, n_runs=iterations
        )

    def _generate_samples(self, max_new_tokens: int = 50, temperature: float = 0.8, top_k: int = 50,
                          model=None, params=None, config=None, model_name: str = "model") -> List[Dict]:
        ...
        # Generate samples using JAX generator
        generator = self._get_text_generator()
        prompts = [
            ("knowledge", "The capital of France is"),
            ("knowledge", "Water boils at"),
            ("reasoning", "If all cats are mammals, and all mammals are"),
        ...
        for category, prompt in prompts:
            generated_text, metadata = generator.generate_from_text(
    ...
    def analyze_health(self) -> Dict:
        ...
        from scripts.analysis.neuron_health_jax import NeuronHealthAnalyzerJAX

        output_dir = self.output_dir / 'health'
        output_dir.mkdir(parents=True, exist_ok=True)

        print("  Analyzing neuron health (forward-based)...")
        analyzer = NeuronHealthAnalyzerJAX(self.model, self.params, self.config)
    ...
    def analyze_embedding(self, n_clusters: int = 5) -> Dict:
        ...
        from scripts.analysis.embedding_jax import EmbeddingAnalyzerJAX

        output_dir = self.output_dir / 'embedding'
        output_dir.mkdir(parents=True, exist_ok=True)

        print(f"  Analyzing neuron embeddings ({n_clusters} clusters)...")
        analyzer = EmbeddingAnalyzerJAX(self.model, self.params, self.config)
    ...
    def analyze_neuron_embedding(self, n_batches: int = 50, k_range: Tuple[int, int] = (5, 20)) -> Dict:
        ...
        from scripts.analysis.embedding_jax import NeuronEmbeddingAnalyzerJAX
        ...
        dataloader = self._get_dataloader()
        analyzer = NeuronEmbeddingAnalyzerJAX(self.model, self.params, self.config)
    ...
    def _extract_checkpoint_config(self, checkpoint_path: str) -> Dict:
        ...
        # For JAX, just return the config we already loaded
        ...
        return {'model': {}, 'training': {}, 'optimizer': {}}

        # TODO: Port PyTorch checkpoint loading to JAX
        config_data = {
            'model': {},
            'training': {},
            'optimizer': {},
    ...
    def _generate_paper_results_json(self, paper_dir: Path, skip_comparison_eval: bool = False):
        ...
        """
        from scripts.analysis.utils_jax import convert_to_serializable
...
Examples:
  ...
  python scripts/analysis/analyze_all.py --checkpoint dawn.pt --val_data val.pt --output results/ --n_batches 200 --max_sentences 5000 --batch_size 32
    ...
    # Device check (JAX auto-detects available accelerators)
    if HAS_JAX:
        devices = jax.devices()
        print(f"JAX devices: {devices}")
    else:
        print("Warning: JAX not available, some features may not work")
