...
def analyze_and_visualize(checkpoint_path, data_path, save_dir='./analysis', n_batches=100, device='cuda'):
    ...
    layer_usage = {ntype: np.zeros((n_layers, n_neurons[ntype])) for ntype in n_neurons}
    cooc_matrix = {ntype: np.zeros((n_neurons[ntype], n_neurons[ntype])) for ntype in n_neurons}
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Process each layer - access routers directly
            for layer_idx, layer in enumerate(model.layers):
                attn = layer.attention

                # Access router through global_routers if available
                if hasattr(model, 'global_routers'):
                    router = model.global_routers.neuron_router

                    # Compute scores using the router
                    # FR scores
                    if hasattr(router, 'compute_feature_r_scores'):
                        fr_scores = router.compute_feature_r_scores(x)
                        _, fr_idx = fr_scores.topk(top_k['FR'], dim=-1)
                        fr_flat = fr_idx.cpu().numpy().flatten()
                        for idx in fr_flat:
                            usage_counts['FR'][idx] += 1
                            layer_usage['FR'][layer_idx, idx] += 1
                        batch_sets['FR'].update(fr_flat)

                    # Other neuron types similar...
                    # For now, use the forward pass with routing info

                # Forward through layer to get x for next layer
                x = layer(x)
    ...
    # Alternative: Use forward pass with routing_info
    print("\nCollecting via forward pass...")
    usage_counts = {ntype: np.zeros(n_neurons[ntype]) for ntype in n_neurons}
    batch_unique = {ntype: [] for ntype in n_neurons}
