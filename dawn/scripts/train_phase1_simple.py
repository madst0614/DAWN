"""
Phase 1 Training Script for DAWN (SimpleDeltaRefiner)

Ablation study: Uses SimpleDeltaRefiner instead of gated DeltaRefiner.
- No GeGLU gates
- Direct delta generation (Transformer blocks only)
- Same architecture otherwise (FFN mountain structure, etc.)

Usage:
    # Use default configuration
    python train_phase1_simple.py

    # Customize parameters directly
    python train_phase1_simple.py --batch_size 32 --base_lr 3e-5 --gradient_clip 0.3

    # Override device or checkpoint directory
    python train_phase1_simple.py --device cuda --checkpoint_dir ./checkpoints_simple
"""
...
def train_single_task(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for step, batch in enumerate(pbar, 1):
            ...
            # Get temperature from gate
            expert = model.experts[expert_name]
            refiner = expert.delta_module.refiner

            # Try mini_gates (hierarchical), final_gate (simple), or gate (legacy)
            gate = None
            if hasattr(refiner, 'mini_gates'):
                gate = refiner.mini_gates[0]
            elif hasattr(refiner, 'final_gate'):
                gate = refiner.final_gate
            elif hasattr(refiner, 'gate'):
                gate = refiner.gate

            if gate and hasattr(gate, 'temperature'):
...
def main():
    ...
    print("\n🔍 Building Phase 1 configuration (SimpleDeltaRefiner)...")
    cfg = DAWNConfig(
        phase=1,
        # Model parameters
    ...
    # Use SimpleDeltaRefiner (no gates)
    cfg.refiner_type = 'simple'
    print("🔧 Using SimpleDeltaRefiner (no gates, direct delta generation)")
